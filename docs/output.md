...
Two optional query parameters narrow the feed. Both take comma-separated lists, and an empty list is the same as leaving the parameter out:

- `icao24`: only messages from these aircraft, e.g. `/stream?icao24=3c6444,4ca7b6`
- `df`: only these downlink formats, e.g. `/stream?df=17,18`

DF19 messages carry no aircraft address, so an `icao24` filter excludes them.
